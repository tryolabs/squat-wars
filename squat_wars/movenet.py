...
from squat_wars.data import BodyPart, Person, person_from_keypoints_with_scores
...
class Movenet(object):
    ...
    def __init__(self, model_name: str) -> None:
        ...
        # Initialize model
        # interpreter = Interpreter(model_path=model_name, num_threads=2)
        interpreter = make_interpreter(model_path_or_content=model_name)
    ...
    def _determine_crop_region(
        ...
        if self._torso_visible(keypoints):
            center_y = (
                target_keypoints[BodyPart.LEFT_HIP][0] + target_keypoints[BodyPart.RIGHT_HIP][0]
            ) / 2
            center_x = (
                target_keypoints[BodyPart.LEFT_HIP][1] + target_keypoints[BodyPart.RIGHT_HIP][1]
            ) / 2
            ...
            return {
                ...
                "height": (crop_corner[0] + crop_length) / image_height - crop_corner[
                    0
                ] / image_height,
                "width": (crop_corner[1] + crop_length) / image_width - crop_corner[
                    1
                ] / image_width,
            }
    ...
    def _crop_and_resize(
        ...
        output_image = cv2.resize(
            output_image, (crop_size[0], crop_size[1]), interpolation=cv2.INTER_NEAREST
        )
    ...
    def _run_detector(
        ...
        self._interpreter.invoke()  # bottleneck here
        ...
        for idx in range(len(BodyPart)):
            keypoints_with_scores[idx, 0] = (
                crop_region["y_min"] + crop_region["height"] * keypoints_with_scores[idx, 0]
            )
            keypoints_with_scores[idx, 1] = (
                crop_region["x_min"] + crop_region["width"] * keypoints_with_scores[idx, 1]
            )
    ...
    def detect(self, input_image: np.ndarray, reset_crop_region: bool = False) -> Person:
        ...
        # the previous frame
        keypoint_with_scores = self._run_detector(  # bottleneck here
            input_image,
            self._crop_region,
